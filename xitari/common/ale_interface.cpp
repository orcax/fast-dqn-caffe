...
void ALEInterface::Impl::getScreenGrayscale(std::vector<pixel_t>& grayscale_output_buffer){
  ALEScreen screen = m_emu->environment->getScreen();
  ...
  m_emu->osystem->colourPalette().applyPaletteGrayscale(&grayscale_output_buffer[0], ale_screen_data, screen_size);
}
...
void ALEInterface::getScreenGrayscale(std::vector<pixel_t>& grayscale_output_buffer){
  m_pimpl->getScreenGrayscale(grayscale_output_buffer);
}
