...
void JointModelDataLayer<Dtype>::LayerSetUp(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  batch_size_ = this->layer_param_.data_param().batch_size();
  num_act_ = this->layer_param_.joint_model_data_param().num_act();
  num_frame_ = this->layer_param_.joint_model_data_param().num_frame();
  channels_ = this->layer_param_.joint_model_data_param().channels();
  //output_clip_ = this->layer_param_.a4_data_param().out_clip();
  //streaming_ = this->layer_param_.a4_data_param().streaming();
  load_to_mem_ = this->layer_param_.joint_model_data_param().load_to_mem();
  output_act_ = this->layer_param_.joint_model_data_param().out_act();
  //output_nextframe_ = this->layer_param_.a4_data_param().out_nextframe();
  output_reward_ = this->layer_param_.joint_model_data_param().out_reward();
  output_value_ = this->layer_param_.joint_model_data_param().out_value();
  //output_life_ = this->layer_param_.a4_data_param().out_life();
  resize_image_ = this->layer_param_.joint_model_data_param().resize_image();
  if (resize_image_) {
    this->height_ = this->layer_param_.joint_model_data_param().resize_height();
    this->width_ = this->layer_param_.joint_model_data_param().resize_width();
    this->size_ = this->channels_ * this->height_ * this->width_;
  }
  if (output_reward_) {
    reward_scale_ = this->layer_param_.joint_model_data_param().reward_scale();
    reward_form_ = this->layer_param_.joint_model_data_param().reward_form();
  }
  if (output_value_) {
    value_form_ = this->layer_param_.joint_model_data_param().value_form();
  }
  //if (output_life_) {
  ...
  int num_out_blobs = 1 + output_act_ + output_reward_ + output_value_;
  ...
  if (output_value_) {
    value_idx_ = output_blob_idx++;
  }
  //if (output_life_) {
  //  life_idx_ = output_blob_idx++;
  //}
  
  this->data_transformer_.reset(new DataTransformer<Dtype>(this->transform_param_, this->phase_));
...
template <typename Dtype>
void JointModelDataLayer<Dtype>::DataLayerSetUp(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  LoadData();
  ...
  if (output_value_) {
    prefetch_value_.mutable_cpu_data();
  }
  //if (output_life_) {
...
template <typename Dtype>
void JointModelDataLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  // 1) image data
  vector<int> data_shape;
  //data_shape.push_back(streaming_ ? num_frame_ : batch_size_);
  //data_shape.push_back(streaming_ ? batch_size_ : num_frame_);
  data_shape.push_back(batch_size_);
  data_shape.push_back(num_frame_);
  data_shape.push_back(channels_);
  data_shape.push_back(height_);
  data_shape.push_back(width_);
  ...
  if (output_act_) {
    ...
    act_shape.push_back(batch_size_);
    act_shape.push_back(num_frame_);
  ...
  if (output_reward_) {
    ...
    reward_shape.push_back(batch_size_);
    reward_shape.push_back(num_frame_);
    if(reward_form_ == 0) {
      reward_shape.push_back(num_act_);
    }
    else if(reward_form_ == 1) {
      reward_shape.push_back(1); // label \in {0, 1, 2} represents {-1, 0, 1}
    }
    else {
      //Nothing
    }
  ...
  // 4) value
  if (output_value_) {
    vector<int> value_shape;
    value_shape.push_back(batch_size_);
    value_shape.push_back(num_frame_);
    if(value_form_ == 0) {
      value_shape.push_back(num_act_);
    }
    else if(value_form_ == 1) {
      value_shape.push_back(1);
    }
    else {
      // Nothing
    }
    top[value_idx_]->Reshape(value_shape);
    prefetch_value_.Reshape(value_shape);
  }
...
template <typename Dtype>
void JointModelDataLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  if (output_value_) {
    caffe_copy(prefetch_value_.count(), prefetch_value_.cpu_data(),
        top[value_idx_]->mutable_cpu_data());
  }
  //if (output_life_) {
...
template <typename Dtype>
void JointModelDataLayer<Dtype>::SampleBatch() {
  sample_batch_.clear();
...
template <typename Dtype>
void JointModelDataLayer<Dtype>::InternalThreadEntry() {
  ...
  Dtype* top_value = NULL;
  if (output_value_) {
    top_value = this->prefetch_value_.mutable_cpu_data();
    caffe_set(this->prefetch_value_.count(), (Dtype)0., top_value);
  }
  ...
  for (int item_id = 0; item_id < batch_size_; ++item_id) {  
    ...
    for (int j = 0; j < num_frame_; ++j) {
      ...
      // 1) data: batch_size x num_frame x c x h x w
      ...
      // 2) action: batch_size x num_frame x num_act
      ...
      if (output_act_/* && j < n_acts*/) {
        ...
        CHECK_LT(acts_[dir_idx].at(act_idx), num_act_) << "Action idx is out of range";
        //top_act[prefetch_act_.offset(streaming_ ? j : item_id, 
        //  streaming_ ? item_id : j) + acts_[dir_idx].at(act_idx)] = 1;
        int offset = prefetch_act_.offset(item_id, j) + acts_[dir_idx].at(act_idx);
        top_act[offset] = 1;
      }

      // 3) reward: 
      // reward_form=0: [batch_size x num_frame x num_act]
      // reward_form=1: [batch_size x num_frame x 3]
      //int n_rewards = output_nextframe_ ? num_frame_ - 1 : num_frame_; // num of rewards in sequence
      if (output_reward_/* && j < num_frame_*/) {
        int reward_idx = img_idx + j;
        CHECK_LT(reward_idx, rewards_[dir_idx].size()) << "Reward index is out of range!";
        float reward = rewards_[dir_idx].at(reward_idx);
        //int offset = prefetch_act_.offset(streaming_ ? j : item_id, streaming_ ? item_id : j);
        //offset += acts_[dir_idx].at(act_idx);
        if(reward_form_ == 0) {
          int offset = prefetch_reward_.offset(item_id, j) + acts_[dir_idx].at(reward_idx);
          top_reward[offset] = reward;
        }
        else if(reward_form_ == 1) {
          int offset = prefetch_reward_.offset(item_id, j);
          if (reward < 0) top_reward[offset] = 0;
          else if (reward == 0) top_reward[offset] = 1;
          else top_reward[offset] = 2;

          //std::cout << item_id << " ";
          //std::cout << j << " ";
          //std::cout << offset << " ";
          //std::cout << reward  << " ";
          //std::cout << top_reward[offset] << std::endl;
        }
        else {
          // Nothing
        }
      ...
      // 4) value
      // value_form=0: [batch_size x num_frame x num_act]
      // value_form=1: [batch_size x num_frame x 1]
      if (output_value_) {
        int value_idx = img_idx + j;
        CHECK_LT(value_idx, values_[dir_idx].size()) << "value index is out of range!";
        float value = values_[dir_idx].at(value_idx);
        //std::cout << dir_idx << " " << value_idx << " " << value << std::endl;

        if(value_form_ == 0) {
          int offset = prefetch_value_.offset(item_id, j) + acts_[dir_idx].at(value_idx);
          top_value[offset] = value;
        }
        else if(value_form_ == 1) {
          int offset = prefetch_value_.offset(item_id, j);
          top_value[offset] = value;
        }
        else {
          // Nothing
...
template <typename Dtype>
void JointModelDataLayer<Dtype>::LoadData() {
  ...
  while (true) {
    ...
    if (output_reward_) {
      ...
      while (getline(file, line)) {
        ...
        if (r != 0) r = r / std::abs(r) * reward_scale_;
        rewards.push_back(r);
      }
      file.close();
    ...
    if (output_value_) {
      std::string value_path(dir_path + "/value.log");
      string line;
      std::ifstream file(value_path.c_str());
      CHECK_EQ(file.is_open(), true) << "Value file not found : " << value_path;
      while (getline(file, line)) {
        float v = atof(line.c_str());
        values.push_back(v);
      }
      file.close();
      this->values_.push_back(values);
    }
...
#ifdef CPU_ONLY
STUB_GPU_FORWARD(JointModelDataLayer, Forward);
#endif

INSTANTIATE_CLASS(JointModelDataLayer);
REGISTER_LAYER_CLASS(JointModelData);
}
