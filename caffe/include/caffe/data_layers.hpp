...
template <typename Dtype>
class A4DataLayer : public BaseDataLayer<Dtype>, public InternalThread {
 public:
  explicit A4DataLayer(const LayerParameter& param)
      : BaseDataLayer<Dtype>(param) {}
  virtual ~A4DataLayer() {}
  // LayerSetUp: implements common data layer setup functionality, and calls
  // DataLayerSetUp to do special data layer setup for individual layer types.
 ...
 protected:
  ...
  Blob<Dtype> prefetch_act_;
  Blob<Dtype> prefetch_clip_;
  Blob<Dtype> prefetch_reward_; //+++
  Blob<Dtype> prefetch_life_; //+++

  bool output_act_;
  bool output_nextframe_; //+++
  bool output_clip_;
  bool output_reward_; //+++
  bool output_life_; //+++
  bool streaming_;
  bool load_to_mem_;
  float scaled_reward_; //+++
  float scaled_life_; //+++
  ...
  std::vector<std::vector<double> > lives_; //+++
  std::vector<BatchIdx> batch_idx_;
  std::vector<BatchIdx> sample_batch_;
};

template <typename Dtype>
class JointModelDataLayer : public BaseDataLayer<Dtype>, public InternalThread {
 public:
  explicit JointModelDataLayer(const LayerParameter& param)
      : BaseDataLayer<Dtype>(param) {}
  virtual ~JointModelDataLayer() {}
  // LayerSetUp: implements common data layer setup functionality, and calls
  // DataLayerSetUp to do special data layer setup for individual layer types.
  // This method may not be overridden.
  // Data layers should be shared by multiple solvers in parallel
